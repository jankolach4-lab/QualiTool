...
## agent_communication:
  - agent: "main"
    message: "AUTO-SYNC PROBLEM GELÖST (15.01.2025): Root-Cause war dass getSession() zu früh aufgerufen wurde, bevor Supabase die persistierte Session aus Storage geladen hat. LÖSUNG: 1) onAuthStateChange Listener registriert in awaitSupabase() - reagiert auf SIGNED_IN/INITIAL_SESSION Events wenn Session tatsächlich verfügbar ist, 2) Listener triggert manualDirectSyncNow() automatisch nach Session-Load mit 1s Verzögerung, 3) awaitSupabase() wird in initializeApp() sofort aufgerufen um Listener früh zu registrieren, 4) Fallback-Interval optimiert (alle 3s statt 5s) mit 3 Retry-Versuchen für getSession(), 5) Duplizierter Code-Block (Zeile 3982-4013) entfernt. Test benötigt echten Supabase-Login oder APK-Build um Session-Load-Event zu verifizieren."
  - agent: "main"
